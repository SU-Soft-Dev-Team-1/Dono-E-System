        ...
        <div class="justify-center items-center flex">
          <div class="w-[130px] h-10 bg-[#1b3c59] rounded-lg shadow border border-[#d4d3d3] flex-col justify-center items-center gap-2 inline-flex">
            <div class="self-stretch grow shrink basis-0 px-6 py-2.5 justify-center items-center gap-2 inline-flex">
              <div class="text-center text-[#f2f2f0] text-base font-normal font-['Ubuntu'] hover:cursor-pointer">
                Remove
              </div>
            </div>
          </div>
  ...
  export default defineComponent({
    setup() {
      ...
      const userItems = computed<Item[]>(() => {
        const filteredItems = itemStore.getItems
          .filter((item: Item) => item.username === userStore.getUsername && item.isListed && !item.isSold && !item.isCart);
